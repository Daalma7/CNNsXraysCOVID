...
Tras encontrar una versión razonablemente buena basándonos en DenseNet121,  utilizaremos como base otros modelos conocidos preentrenados también en ImageNet, y emplearemos las misma modificaciones sobre ellos que las que hemos empleado sobre DenseNet121, para poder hacer valoraciones y comparaciones entre los mismos.\\
...
Podemos comprobar que efectivamente el modelo no es para nada satisfactorio. Obtenemos un valor de test accuracy cercano al 33\%, que sería justo lo equivalente a elegir una de las tres clases de forma aleatoria sin ningún tipo de criterio.
...
Podemos ver que la mejora ha sido bastante efectiva, obteniendo un valor para test accuracy de 0.9597. Sin embargo, si nos fijamos en las gráficas, puede ser que tengamos un poco de overfitting puesto que el validation loss no decrece al mismo ritmo que el training loss.\\
...
Con la regularización hemos conseguido mantener prácticamente la precisión que teníamos, pero hemos reducido un poco el overfitting que había, como podemos comprobar en las gráficas.\\
...
Los resultados obtenidos son bastante buenos, superando el 97\% de test accuracy. La matriz de confusión nos sigue indicando que el único pequeño problema que sigue teniendo son esas 14 imágenes de la clase NORMAL clasificadas como Viral Pneumonia, que aunque se ha reducido respecto a las versiones iniciales, ha sido una constante en todo el modelo.\\


Ahora vamos a aplicar el modelo de la versión 2, donde utilizábamos los pesos fijos, y este último modelo de fine tunning, pero partiendo de otras arquitecturas distintas a DenseNet.


\subsection{ResNet50}

Para la primera versión con pesos congelados tenemos lo siguiente:
...
\end{tabular}
\caption{Resultados de la ejecución para la red ResNet, mostrando los anteriores}
\end{table}
...
\end{tabular}
\end{center}
\caption{Matriz de confusión para la red ResNet}
\end{table}


El resultado obtenido es bueno, pero no se acerca al anterior de DenseNet. \\
...
Primero vemos la versión usando VGG16 como extractor de características:
...
\end{tabular}
\caption{Resultados de la ejecución para la red VGG16, mostrando los anteriores}
\end{table}
...
\end{tabular}
\end{center}
\caption{Matriz de confusión para la red VGG16}
\end{table}
...
\caption{Resultados de la ejecución para la red InceptionV3, mostrando los anteriores}
\end{table}
...
\end{tabular}
\end{center}
\caption{Matriz de confusión para la red InceptionV3}
\end{table}


En general es un buen resultado, aunque no es excepcional comparado con algunos anteriores.\\
...
Ahora sí que hemos dado un salto de calidad respecto al anterior, que está a la altura del mejor modelo hasta el momento (DN-V3). La única dificultad que encuentra el modelo está a la hora de clasificar imágenes de la clase Neumonía Viral, confunciéndolas con imágenes de pulmones sanos.

\subsection{Xception}

Versión con la red congelada:
...
\hline
\end{tabular}
\caption{Resultados de la ejecución para la red Xception, mostrando los anteriores}
\end{table}
...
\end{tabular}
\end{center}
\caption{Matriz de confusión para la red Xception}
\end{table}

Buenos resultados, que recuerdan bastante a la versión de InceptionV3, que tiene sentido al ser modelo similares.\\
...
Observando la matriz de confusión vemos que, al igual que InceptionV3,  8 de los 9 errores han sido por predecir la clase Normal cuando era Neumonía Vírica. Esto contrasta con el que ha sido nuestro segundo mejor modelo, el de DenseNet con fine tunning, que predecía Neumonía Vírica cuando era Normal.

\subsection{Red from scratch}

FALTA EXPLICAR UN POCO LA ARQUITECTURA (COPIA Y PEGA DE TU P2 Y LISTO)
...
Vamos a ver el mapa de calor para dos de los modelos anteriores. En concreto para ResNet50 en su versión con los pesos fijos y para el mejor que hemos obtenido, Xception con fine tunning.

Para Resnet obtenemos lo siguiente:
...
Si comparamos las dos salidas obtenidas, podemos apreciar claramente una diferencia notable. En el primer caso, al estar los pesos de las capas convolucionales congelados, el modelo no se adapta al tipo de imagen que recibe, de ahí que la salida no tenga mucho sentido.\\

En cambio, para el segundo modelo podemos apreciar que la red se centra en la zona central de las imágenes, que es justo el lugar de ella donde aparece la información más valiosa, evitando los bordes y esquinas donde la información es la misma en todas las clases.
